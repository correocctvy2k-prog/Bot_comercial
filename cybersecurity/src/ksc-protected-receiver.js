...
const { importKscProtectedInventory } = require('./ksc-protected-importer');
...
function receiveKscProtectedOnce({
  db,
  incomingDirectory,
  processingDirectory,
  acceptedDirectory,
  ...
  for (const name of candidates) {
    ...
    const claimName = `.claimed-${process.pid}-${crypto.randomBytes(6).toString('hex')}-${safeName}`;
    const claimPath = path.join(incomingDirectory, claimName);
    ...
    try {
      ...
      const result = importKscProtectedInventory({
        ...
        importedAt,
        custodyReference: `restricted://ksc-sftp/${hash}`,
      });
      ...
      if (fs.existsSync(claimPath)) {
        if (!evidenceStored) {
          fs.copyFileSync(
            claimPath,
            path.join(rejectedDirectory, `${rejectionId}.rejected`),
...
module.exports = { MAX_EXPORT_BYTES, readClaimedFile, receiveKscProtectedOnce };
