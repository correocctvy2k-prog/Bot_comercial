...
// spec 0016 — puerto funcional de CRM_Frontend/Table Trello/backend/src/services/excel.service.js
// (lógica ya probada: matching difuso de puntos, recálculo de fórmulas, detección de archivo
// bloqueado), adaptado para cctv-automation-final: sin clase, sin acoplar a la SQLite de
// "Table Trello" (ese proyecto queda intacto y sin tocar). Quien llama a marcarMantenimiento
// decide cómo auditar el resultado (aquí: platform/import-trello-maintenance.js -> audit_log).
...
function getSearchKeys(value = '') {
  ...
  for (const candidate of candidates) {
    for (const variant of withRomanVariants(candidate)) keys.add(variant);
  }
...
function enableWorkbookRecalculation(workbookXml) {
  ...
  if (/<calcPr\b[^>]*\/>/.test(workbookXml)) {
    return workbookXml.replace(/<calcPr\b[^>]*\/>/, (match) => {
      const next = match.replace(/\s(?:calcMode|fullCalcOnLoad|forceFullCalc)="[^"]*"/g, '');
...
async function patchNumericCellInWorkbook(filePath, sheetName, cellAddress, value, formulaCacheValues = {}) {
  ...
  const tempPath = path.join(path.dirname(filePath), `~$cctv-${Date.now()}-${path.basename(filePath)}`);
...
async function loadWorkbook(filePath) {
  ...
  if (!worksheet) throw new Error(`No existe la hoja "${SHEET_NAME}" en ${filePath}`);
  return { workbook, worksheet };
}

function getZoneBlocks(worksheet) {
  const blocks = [];
...
function getPoints(worksheet, zoneFilter = null) {
  const zoneKey = zoneFilter ? normalize(zoneFilter) : null;
  const blocks = getZoneBlocks(worksheet).filter((block) => !zoneKey || block.zoneKey === zoneKey);
  const points = [];
...
function findPoint(worksheet, nombrePunto, zona = null) {
  ...
  const points = getPoints(worksheet, zona);
  let matches = pointCode ? points.filter((point) => point.codigo === pointCode) : [];

  if (matches.length === 0) matches = points.filter((point) => searchKeys.includes(point.puntoKey));
  if (matches.length === 0) matches = points.filter((point) => compactSearchKeys.includes(compactKey(point.puntoKey)));
...
function getCellNumberForFormula(worksheet, address, overrides = {}) {
...
function evaluateWorksheetFormula(formula, worksheet, overrides = {}) {
  const expression = formula.replace(/^=/, '').trim();
  const sumMatch = expression.match(/^SUM\(([^)]+)\)$/i);
  ...
  if (sumMatch) {
    return sumMatch[1].split(',').reduce((total, range) => {
      return total + rangeAddresses(range.trim()).reduce((rangeTotal, address) => {
        return rangeTotal + getCellNumberForFormula(worksheet, address, overrides);
      }, 0);
    }, 0);
  }
  ...
  const numericExpression = expression.replace(/\b[A-Z]+\d+\b/gi, (address) => {
    return String(getCellNumberForFormula(worksheet, address.toUpperCase(), overrides));
  });
  if (!/^[0-9+\-*/().\s]+$/.test(numericExpression)) return null;
...
function findTotalRowForPoint(worksheet, point, period) {
  ...
  for (let row = point.row + 1; row <= worksheet.rowCount; row += 1) {
    ...
    if (formula && /^SUM\(/i.test(formula) && formula.includes(periodColumn)) return row;
  }
  return null;
}

function getFormulaCacheValuesForPoint(worksheet, point, changedAddress, changedValue) {
  const totalRow = findTotalRowForPoint(worksheet, point, Object.keys(point.periodColumns)[0]);
  if (!totalRow) return {};
  ...
  for (const periodColumn of Object.values(point.periodColumns)) {
    ...
    if (totalFormula) {
      const total = evaluateWorksheetFormula(totalFormula, worksheet, overrides);
      if (total != null) { cacheValues[totalAddress] = total; overrides[totalAddress] = total; }
    }
  }
  ...
  for (const periodColumn of Object.values(point.periodColumns)) {
    ...
    if (percentFormula) {
      const percent = evaluateWorksheetFormula(percentFormula, worksheet, overrides);
      if (percent != null) { cacheValues[percentAddress] = percent; overrides[percentAddress] = percent; }
    }
  }
  ...
  for (const periodColumn of Object.values(point.periodColumns)) {
    ...
    if (grandTotalFormula) {
      const grandTotal = evaluateWorksheetFormula(grandTotalFormula, worksheet, overrides);
      if (grandTotal != null) { cacheValues[grandTotalAddress] = grandTotal; overrides[grandTotalAddress] = grandTotal; }
    }
  }
...
/** Marca (o limpia, con valor null) la celda de mantenimiento de un punto para el período de
 *  `fecha` (o el explícito). `zona`, si se pasa, evita ambigüedad cuando dos puntos comparten
 *  nombre en zonas distintas -- se recomienda pasar el nombre/zona canónicos de `locations`
 *  cuando el ítem de Trello ya tiene identidad resuelta (más confiable que el nombre crudo). */
async function marcarMantenimiento({ filePath, nombrePunto, zona = null, periodo = null, fecha = new Date(), valor = 1 }) {
  const finalPeriodo = periodo || getPeriodoFromDate(fecha);
  if (!PERIODS.includes(finalPeriodo)) throw new Error(`Periodo inválido: ${finalPeriodo}. Use R1, R2 o R3.`);

  const { worksheet } = await loadWorkbook(filePath);
  const point = findPoint(worksheet, nombrePunto, zona);
  const columnLetter = point.periodColumns[finalPeriodo];
  const cellAddress = `${columnLetter}${point.row}`;
  const previousValue = worksheet.getCell(cellAddress).value;

  const formulaCacheValues = getFormulaCacheValuesForPoint(worksheet, point, cellAddress, valor);
  await patchNumericCellInWorkbook(filePath, SHEET_NAME, cellAddress, valor, formulaCacheValues);

  return { punto: point.punto, zona: point.zona, periodo: finalPeriodo, celda: cellAddress, valor, previousValue };
}

async function getResumen(filePath) {
  const { worksheet } = await loadWorkbook(filePath);
  const blocks = getZoneBlocks(worksheet);
  const points = getPoints(worksheet);
  const resumen = blocks.map((block) => {
    ...
    for (const period of PERIODS) {
      ...
      periods[period] = { programados: zonePoints.length, realizados: completed, porcentaje: zonePoints.length ? Math.round((completed / zonePoints.length) * 100) : 0 };
    }
    return { zona: block.zone, puntos: zonePoints.length, periodos: periods };
  });
  return { filePath, sheet: SHEET_NAME, zonas: resumen };
}
