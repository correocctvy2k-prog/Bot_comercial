                                ...
                                {analytics?.cctv_behavior && (
                                    <div className="col-span-2 rounded-lg border border-blue-500/20 bg-blue-500/5 p-2.5">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                                            {[
                                                ['1ª CCTV', analytics.cctv_behavior.emailOpening, 'text-emerald-400'],
                                                ['Últ. CCTV', analytics.cctv_behavior.emailClosing, 'text-blue-400'],
                                                ['1er ping', analytics.cctv_behavior.firstPing, 'text-violet-400'],
                                                ['Últ. ping', analytics.cctv_behavior.lastPing, 'text-violet-300']
                                            ].map(([label, value, tone]) => (
                                                <div key={label} className="rounded-md bg-background/50 px-2 py-1.5">
                                                    <span className="block text-[9px] text-muted-foreground">{label}</span>
                                                    <b className={`text-xs ${value ? tone : 'text-muted-foreground'}`}>
                                                        {value ? new Date(value).toLocaleTimeString('es-CO', { timeZone: 'America/Bogota', hour: '2-digit', minute: '2-digit' }) : 'Sin dato'}
                                                    </b>
                                                </div>
                                            ))}
                                        </div>
