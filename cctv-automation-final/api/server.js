...
function dailyEventsData(dateValue){
  ...
  const pointMap=new Map();for(const item of items.filter(row=>['OPENING','CLOSING'].includes(row.eventType))){for(const target of targetsFor(item)){const key=target.targetId||`RAW:${item.payload.storeRaw||'UNKNOWN'}`;const point=pointMap.get(key)||{key,locationId:target.targetId,name:target.targetName||item.payload.storeRaw||'Por identificar',zone:target.targetZone||null,opening:null,closing:null,linked:!!target.targetId,sharedOpeningGroup:target.openingGroup||null};const stamp=item.occurredAt||item.receivedAt;if(item.eventType==='OPENING'&&(!point.opening||stamp<point.opening))point.opening=stamp;if(item.eventType==='CLOSING'&&(!point.closing||stamp>point.closing))point.closing=stamp;pointMap.set(key,point);}}const pointOperations=[...pointMap.values()].map(point=>({...point,status:point.opening&&point.closing?'COMPLETE':point.opening?'OPEN_ONLY':'CLOSE_ONLY'})).sort((a,b)=>(a.status==='COMPLETE')-(b.status==='COMPLETE')||a.name.localeCompare(b.name));
  ...
  for(const item of items.filter(row=>row.payload.hasAttachment&&['OPENING','CLOSING'].includes(row.eventType))){const key=`${item.eventType}|${identityKey(item)}`,existing=evidenceMap.get(key);if(!existing||(item.eventType==='OPENING'&&eventStamp(item)<eventStamp(existing))||(item.eventType==='CLOSING'&&eventStamp(item)>eventStamp(existing)))evidenceMap.set(key,item);}
  ...
  for(const item of items.filter(row=>row.payload.hasAttachment&&!['OPENING','CLOSING','MOTION','MOVIMIENTO','DISCARDED'].includes(row.eventType))){const key=`${identityKey(item)}|${item.eventType}`,group=additionalGroups.get(key)||[];group.push(item);additionalGroups.set(key,group);}
  ...
  for(const item of [...evidenceMap.values()].sort((a,b)=>eventStamp(a)-eventStamp(b))){
    const prior=correlatedEvidence.at(-1),sameLocation=prior?.locationId&&item.locationId===prior.locationId,openingTripwire=new Set([prior?.eventType,item.eventType]).has('OPENING')&&new Set([prior?.eventType,item.eventType]).has('TRIPWIRE');
