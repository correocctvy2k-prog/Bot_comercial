...
- [ ] Al marcar/desmarcar un ítem en la lista Trello de mantenimiento, dentro de ~1-2 minutos
      la celda correspondiente en el Excel cambia (verificable una vez montado el CIFS).
- [ ] `GET /api/cctv/maintenance/excel-status` responde si el archivo está configurado,
      accesible, y si está bloqueado (y por quién, si se puede determinar).
- [ ] Pestaña Mantenimiento muestra la ruta de red con botón "copiar ruta" y el estado de
      bloqueo cuando aplica.
- [ ] Si el archivo está bloqueado o la ruta no está montada, el ciclo de importación de Trello
...
- [ ] `cctv-automation-final`: `npm test` en verde (tests nuevos para matching de puntos,
      direccionamiento de celdas, y detección de bloqueo).
- [ ] Runbook de montaje CIFS entregado y claro, con los comandos exactos para `.65`.
