...
const CircleGauge = ({ label, value }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-2 rounded-xl bg-background/50 border border-border/30 flex-1 min-w-[72px] shadow-sm">
      <div className="relative w-12 h-12 flex items-center justify-center">
        ...
        <span className="absolute text-[11px] font-black text-foreground">{displayVal}</span>
      </div>
      <span className="mt-1.5 text-[9px] font-black uppercase tracking-wider text-muted-foreground">{label}</span>
    </div>
  );
};
...
export default function ServicesTIDashboard() {
  ...
  const [layoutOrder, setLayoutOrder] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("skylab.nodeMonitor.layout.v1") || "[]");
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    loadState();
    const interval = setInterval(() => loadState(), 120000); // Poll every 120 seconds
  ...
  const handleMove = (id, direction) => {
    ...
    // Populate layout order if not set
    ...
    localStorage.setItem("skylab.nodeMonitor.layout.v1", JSON.stringify(currentOrder));
  };
  ...
  const sortedTargets = useMemo(() => {
    ...
    // Append any newly discovered targets
  ...
  const openModal = (target = null) => {
    ...
    if (target) {
      setFormFields({
        ...
        password: "", // empty means unmodified
  ...
  const handleSaveServer = async (e) => {
    ...
    if (selectedTarget && !payload.password) {
      delete payload.password; // Do not overwrite if empty on edit
    }
  ...
  const handleDeleteServer = async () => {
    ...
    try {
      ...
      setIsEditModalOpen(false);
      // Remove from layoutOrder if present
      const nextOrder = layoutOrder.filter(id => id !== selectedTarget.id);
      setLayoutOrder(nextOrder);
      localStorage.setItem("skylab.nodeMonitor.layout.v1", JSON.stringify(nextOrder));
  ...
  // Force backend check sweep
  const handleSweep = async () => {
    setRefreshing(true);
    const toastId = toast.loading("Realizando escaneo de servidores...");
    try {
      await servicesTIService.triggerSweep();
      // Delay state load slightly to allow SSH sweeps to progress
      setTimeout(async () => {
        await loadState();
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {criticalAlerts.length > 0 && (
        <div className="relative overflow-hidden rounded-xl border border-rose-500/30 bg-rose-500/5 p-4 flex items-center gap-4 animate-pulse">
          <div 
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 items-start">
        ...
        {/* Left Side: Smart Alerts Sidebar if any exist */}
        {state?.smartAlerts && state.smartAlerts.length > 0 && (
          <section className="lg:col-span-3 space-y-4 rounded-xl border border-border bg-card/25 p-4 backdrop-blur-sm">
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
              {state.smartAlerts.slice(0, 5).map(alert => (
        ...
        {/* Main Grid: Server Cards */}
        <section className={`${state?.smartAlerts && state.smartAlerts.length > 0 ? "lg:col-span-9" : "lg:col-span-12"} grid grid-cols-1 md:grid-cols-2 gap-4`}>
          {sortedTargets.length > 0 ? (
            sortedTargets.map((target) => {
              ...
              // Check filesystem alerts for specific indicators
              ...
              // Local alerts summary
              const alerts = res?.alerts || [];

              return (
                <article key={target.id} className={`rounded-xl border bg-card/35 p-4 backdrop-blur-sm flex flex-col transition-all hover:border-border/80 ${status === "offline" ? "border-rose-500/20 bg-rose-500/5" : "border-border/50"}`}>
                  ...
                  <header className="flex items-start justify-between gap-3 border-b border-border/40 pb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span 
                          className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                            ...
                            status === "degraded" ? "bg-amber-400 animate-[breathe-amber_3s_ease-in-out_infinite]" :
                            status === "offline" ? "bg-rose-500 animate-[breathe-red_2s_ease-in-out_infinite]" :
                            "bg-slate-600"
                          }`}
                          style={{
                    ...
                    <div className="flex items-center gap-1">
                      <button 
                        ...
                        className="p-1 rounded bg-background hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
                        title="Mover Arriba"
                      >
                        <ChevronUp size={12} />
                      </button>
                      ...
                      <button 
                        ...
                        className="p-1 rounded bg-background hover:bg-muted text-muted-foreground hover:text-foreground transition-colors ml-1"
                        title="Editar"
                      >
                        <Edit size={12} />
                      </button>
                  ...
                  {(target.tags || []).length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2.5">
                      {target.tags.map((tag, idx) => (
                        <span key={idx} className="inline-flex items-center gap-1 rounded bg-primary/10 border border-primary/20 px-1.5 py-0.5 text-[9px] font-black uppercase text-primary">
                          <Tag size={8} />
                          {tag}
                        </span>
                      ))}
                  ...
                  {/* Nav tabs */}
                  <nav className="flex border-b border-border/20 mt-3.5 text-xs font-bold">
                    <button
                    ...
                    {activeTab === "metrics" && (
                      <div className="space-y-3.5">
                        ...
                        {alerts.length > 0 && (
                          <div className="space-y-1.5">
                            {alerts.map((alert, idx) => (
                              <div key={idx} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold bg-rose-500/10 border border-rose-500/20 text-rose-400">
                        ...
                        {target.enabled && status === "online" && history[target.id] && history[target.id].length >= 2 && (
                          <div className="rounded-xl border border-border/30 bg-background/30 p-2.5">
                            <div className="flex items-center justify-between text-[9px] font-bold text-muted-foreground uppercase tracking-wider mb-2">
                              ...
                              <div className="flex gap-3">
                                <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded bg-emerald-400" /> CPU: {Math.round(metrics.cpu.usagePercent)}%</span>
                                <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded bg-sky-400" /> RAM: {Math.round(metrics.memory.usedPercent)}%</span>
                              </div>
                            </div>
                            <div className="h-14">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs border-t border-border/20 pt-3">
                          <div className="p-2 rounded bg-background/25 border border-border/20">
                            <span className="text-[9px] font-black text-muted-foreground uppercase block">Latencia</span>
                            <strong className="text-foreground">{res?.tcp?.latencyMs ? `${res.tcp.latencyMs} ms` : "N/D"}</strong>
                          </div>
                          <div className="p-2 rounded bg-background/25 border border-border/20">
                            <span className="text-[9px] font-black text-muted-foreground uppercase block">Uptime</span>
                            <strong className="text-foreground truncate block">{formatUptimeDays(metrics?.uptime)}</strong>
                          </div>
                          <div className="p-2 rounded bg-background/25 border border-border/20">
                            <span className="text-[9px] font-black text-muted-foreground uppercase block">Load Avg</span>
                            <strong className="text-foreground">{metrics?.cpu ? `${metrics.cpu.load1} / ${metrics.cpu.load5}` : "N/D"}</strong>
                          </div>
                          <div className="p-2 rounded bg-background/25 border border-border/20">
                            <span className="text-[9px] font-black text-muted-foreground uppercase block">Cores / OS</span>
                            <strong className="text-foreground truncate block">{metrics?.cpu?.cores ? `${metrics.cpu.cores} Cores` : "N/D"}</strong>
                          </div>
                        </div>

                        {/* SSH Diagnostic Info */}
                        {res?.status === "degraded" && (
                          <div className="flex gap-2.5 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs">
                      ...
                      <div className="space-y-3">
                        <div className="flex gap-2 justify-end">
                          <button
                            ...
                            className={`px-2 py-0.5 rounded text-[10px] font-bold border transition-colors ${sortDirection === "desc" ? "bg-primary/20 border-primary text-primary" : "bg-transparent border-border text-muted-foreground"}`}
                          >
                            Mayor uso
                          </button>
                          <button
                            onClick={() => setDiskSorts(prev => ({ ...prev, [target.id]: "asc" }))}
                            className={`px-2 py-0.5 rounded text-[10px] font-bold border transition-colors ${sortDirection === "asc" ? "bg-primary/20 border-primary text-primary" : "bg-transparent border-border text-muted-foreground"}`}
                          >
                            Menor uso
                          </button>
                        </div>
                        ...
                        <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
                          {(() => {
                            ...
                            return sorted.map((fs, idx) => {
                              ...
                              return (
                                <div key={idx} className="space-y-1" title={fs.filesystem || fs.name}>
                                  <div className="flex justify-between text-xs font-medium">
                                    <span className="font-bold text-foreground/90 truncate max-w-[120px]">{fs.name || fs.mount}</span>
                                    <span className="text-muted-foreground text-[10px]">{fs.used} / {fs.size} (L: {fs.available})</span>
                                  </div>
                                  <div className="w-full h-3 bg-background/80 border border-border/20 rounded-full overflow-hidden relative">
                                    <div 
                                      className={`h-full ${barTone} transition-all duration-500 flex items-center justify-end pr-1.5`} 
                                      style={{ width: `${Math.max(0, Math.min(100, pct))}%` }}
                                    >
                                      {pct > 20 && <span className="text-[8px] font-black text-black">{Math.round(pct)}%</span>}
                                    </div>
                                    {pct <= 20 && <span className="absolute right-1.5 top-0 text-[8px] font-black text-muted-foreground">{Math.round(pct)}%</span>}
                                  </div>
                                </div>
                              );
                            });
                      ...
                      <div className="space-y-3.5 max-h-[300px] overflow-y-auto pr-1">
                        ...
                        {hasDocker ? (
                          <div className="space-y-2">
                            <h4 className="text-[10px] font-black uppercase tracking-wider text-muted-foreground flex items-center gap-1.5 border-b border-border/20 pb-1">
                            ...
                            <div className="space-y-1.5">
                              {metrics.docker.containers.map((container, idx) => {
                                ...
                                return (
                                  <div 
                                    key={idx} 
                                    className={`p-2 rounded bg-background/25 border text-[11px] ${
                                      isHot ? "border-rose-500/20 bg-rose-500/5" : "border-border/20"
                                    }`}
                                  >
                                    <div className="flex items-center justify-between gap-2">
                                      <div className="flex items-center gap-1.5 min-w-0">
                                        <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                                          ...
                                          status === "restarting" ? "bg-amber-400" : "bg-rose-500"
                                        }`} />
                        ...
                        {hasShareplex ? (
                          <div className="space-y-2 border-t border-border/20 pt-3">
                            ...
                            <div className="flex items-center justify-between p-2 rounded bg-background/25 border border-border/20 text-xs">
                              <div className="flex items-center gap-2">
                              ...
                              <div className="text-right">
                                ...
                                <span className="block text-[10px] text-muted-foreground">{metrics.shareplex.processCount || 0} procesos</span>
                              </div>
                            </div>
                          </div>
                        ) : null}
                        ...
                        {!hasDocker && !hasShareplex ? (
                          <p className="text-xs text-muted-foreground text-center py-6">No se detectaron servicios (Docker o SharePlex) en este servidor.</p>
                        ) : null}
                      </div>
                    )}
                  </div>
            ...
            <div className="col-span-2 rounded-xl border border-dashed border-border p-12 text-center text-muted-foreground">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              {loadingAnalysis ? (
                <div className="flex flex-col items-center justify-center py-20 gap-3">
                  ...
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Ejecutando análisis...</p>
                </div>
              ) : analysisData ? (
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    {/* Containers consuming most resources */}
                    <div className="space-y-2 col-span-1 md:col-span-2">
                      <h3 className="text-xs font-black text-muted-foreground uppercase tracking-widest flex items-center gap-1.5 border-b border-border/20 pb-1.5">
                    ...
                    <div className="space-y-2 col-span-1 md:col-span-2">
                      ...
                      <div className="space-y-2">
                        {analysisData.shareplex?.length > 0 ? (
                          analysisData.shareplex.map((sh, idx) => (
                            <div key={idx} className="flex justify-between items-center text-xs p-2.5 rounded bg-background/40 border border-border/20">
                              <div>
                                <strong className="font-bold text-foreground block">{sh.serverName}</strong>
                                <span className="text-[10px] text-muted-foreground">Puertos asignados: {(sh.ports || []).join(", ") || "Ninguno"}</span>
                              </div>
                              <span className={`text-xs font-black uppercase ${sh.running ? "text-emerald-400" : "text-rose-400"}`}>
                                {sh.running ? "Activo" : "Detenido"}
                              </span>
                            </div>
                          ))
                        ) : <p className="text-xs text-muted-foreground py-4 text-center">No se detectaron replicadores SharePlex en ejecución.</p>}
                      </div>
                    </div>
                  </div>
                </>
              ) : (
                <p className="text-xs text-muted-foreground text-center py-10">No se pudieron cargar los datos analíticos de la base.</p>
              )}
            </div>

            {/* Footer */}
            <div className="p-4 bg-background border-t border-border/50 flex justify-end">
              <button
                onClick={() => setIsAnalysisModalOpen(false)}
                className="px-5 py-2.5 bg-primary text-primary-foreground font-black uppercase text-xs rounded-xl hover:shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-all active:scale-95"
              >
                Entendido
              </button>
            </div>
